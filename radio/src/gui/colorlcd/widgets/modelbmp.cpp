  ...
  public:
    ...
    void refresh(BitmapBuffer * dc) override
    {
      ...
      if (rect.h >= 96 && rect.w >= 120) {
        ...
        dc->drawSizedText(5, 5, g_model.header.name, LEN_MODEL_NAME, fontsize | CUSTOM_COLOR);
      }
      // smaller space to draw
        ...
        if (!filename.empty() && buffer) {
          ...
          dc->drawSizedText(0, 0, g_model.header.name, LEN_MODEL_NAME, fontsize | CUSTOM_COLOR);
        }
      }
    }
